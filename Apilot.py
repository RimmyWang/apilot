...
class Apilot(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if content == "笑话":
            joke = self.get_joke()
            reply = self.create_reply(ReplyType.TEXT, joke)
            e_context["reply"] = reply
            e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
            return
    ...
    def get_help_text(self, verbose=False, **kwargs):
        ...
        help_text += "  🔥 舔狗: 发送“舔狗”获取舔狗日记。\n"
        help_text += "  😃 笑话: 发送“笑话”获取舔狗日记。\n"

        # 查询类
        help_text += "\n🔍 查询工具：\n"
        help_text += "  🌦️ 天气: 发送“城市+天气”查天气，如“北京天气”。\n"
        help_text += "  📦 快递: 发送“快递+单号”查询快递状态。如“快递112345655” 或快递11233445:手机尾号\n"
    ...
    def query_express_info(self, alapi_token, tracking_number, com="", order="asc"):
        ...
        if ":" in tracking_number:
            tracking_number, phone = tracking_number.split(":")
        ...
        headers = {"Content-Type": "application/json"}  # 修改请求头为 application/json
        
        try:
            logger.debug(f"[Apilot] 快递查询请求参数: {params}")
            response_json = self.make_request(url, method="GET", headers=headers, params=params)
            logger.debug(f"[Apilot] 快递查询API响应: {response_json}")
            ...
            if code != 200:
                ...
                logger.error(f"[Apilot] 快递查询失败: code={code}, msg={msg}")
                return f"查询失败: {msg}"
                
            data = response_json.get("data", None)
            if not data or not data.get("info"):
                return "未查询到快递信息，请检查单号是否正确"
            ...
            logger.error(f"[Apilot] 快递查询异常: {str(e)}")
            return f"快递查询失败，请稍后重试"
    ...
    def get_dog_diary(self):
        url = BASE_URL_VVHAN + "text/dog?type=json"
        try:
            response = self.make_request(url, method="GET")
            # 记录响应内容用于调试
            logger.debug(f"[Apilot] get_dog_diary response: {response}")
            ...
            if isinstance(response, dict):
                ...
                if response.get('success', False):
                    ...
                    if data:
                        # 检查data是否为字典类型，并从content字段获取内容
                        if isinstance(data, dict) and 'content' in data:
                            return data['content']
                        # 如果data是字符串类型，直接返回
                        elif isinstance(data, str):
                            return data.strip()
                # 如果没有success字段或为False，尝试获取msg字段
                return response.get('msg', '获取舔狗日记失败，请稍后再试')
            
            # 如果响应不是字典类型，可能是字符串或其他类型
            if isinstance(response, str):
                return response.strip()
                
            return '获取舔狗日记失败，请稍后再试'
        except Exception as e:
            logger.error(f"[Apilot] get_dog_diary error: {str(e)}")
            return self.handle_error(e, "获取舔狗日记失败，请稍后再试")
            
    def get_joke(self):
        url = BASE_URL_VVHAN + "text/joke?type=json"
        try:
            response = self.make_request(url, method="GET")
            # 记录响应内容用于调试
            logger.debug(f"[Apilot] get_joke response: {response}")
                ...
                if response.get('success', False):
                    ...
                    if data:
                        ...
                        # 如果data是字符串类型，直接返回
                            ...
                            return data.strip()
                # 如果没有success字段或为False，尝试获取msg字段
                return response.get('msg', '获取笑话失败，请稍后再试')
            ...
            logger.error(f"[Apilot] get_joke error: {str(e)}")
            return self.handle_error(e, "获取笑话失败，请稍后再试")
